...
desc "Run all erlang unit tests"
task :all_tests => [:applications] + ERL_BEAM_TESTS do |t|
...
desc "Run erlang unit test for a specific application"
...
task :edoc, :name, :needs => [:applications] do |t,args|
  run_script("make_doc",[args.name] + ERL_DIRECTORIES)
end

desc "Buid all application documentation"
task :edocs => [:applications] do |t,args|
  ERL_APPLICATIONS.each do |application|
...
task :compile => [:modules, :applications, :all_tests]
